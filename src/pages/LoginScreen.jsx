...
export default function LoginScreen() {
  ...
  const { signIn, signInWithGoogle, loginAsDemo } = useAuth()
  ...
  return (
    <div className="min-h-screen bg-dark-bg flex flex-col px-6 py-8 relative">
      ...
      <p className="text-sm text-muted mb-8">Access your fleet telemetry and logs</p>
      ...
      <form onSubmit={handleLogin} className="space-y-4 flex-1">
        ...
        <div className="space-y-1.5">
          <label className="text-caps text-muted">Email</label>
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between items-center">
            ...
            <button type="button" className="text-xs text-accent font-medium">Forgot Password?</button>
          </div>
          <div className="relative">
        ...
        <button
          type="button"
          onClick={handleDemo}
          className="w-full flex items-center justify-center gap-2 bg-accent/10 border border-accent/20 py-3.5 rounded-2xl font-semibold text-sm text-accent active:scale-[0.98] transition-transform mt-2"
        >
          🚀 Try Demo Mode
        </button>
      </form>
      ...
      <p className="text-center text-sm text-muted mt-6">
        ...
        <Link to="/signup" className="text-accent font-semibold">Sign up</Link>
      </p>
    </div>
  )
}
